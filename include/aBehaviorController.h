...
public:
	...
	bool isLeader() const { return mLeader; }

	virtual void display();

    const AJoint& getGuide() const { return m_Guide;  }   // the guide determines the agent root position and orientation

	vec3 getPosition() const { return  m_Guide.getLocalTranslation(); return m_Pos0; }  // gets the global position of agent
    vec3 getVelocity() const { return m_Vel0;  }
	vec3 getOrientation() const { vec3 angle(0.0, m_state[ORI][_Y], 0.0); return angle; }  // gets global orientation of agent

	void setTarget(AJoint& target);  // sets the target for a behavior
	const AJoint& getTarget() const { return m_Target; }
	vec3 getDesiredVelocity() const { return m_Vdesired; }

	void setBehavior(Behavior* behavior);   // sets the active behavior
	Behavior* getBehavior() const { return mpActiveBehavior; }

	void createBehaviors(std::vector<BehaviorController>& agentList, std::vector<Obstacle>& obstacleList); 
	BehaviorType getActiveBehavior() { return m_BehaviorType; }  // new
	void setActiveBehavior(BehaviorType type);  // new
...
protected:
	...
	vector<BehaviorController>* m_AgentList;  // a list of all the other agents in the world
	vector<Obstacle>* mObstacleList;          // a list of all the obstacles in the world
	// end new
